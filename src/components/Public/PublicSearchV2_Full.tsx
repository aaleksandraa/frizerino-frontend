...
export const PublicSearchV2: React.FC = () => {
  ...
  // Load available services from popular services
  useEffect(() => {
    if (popularServices.length > 0) {
      const serviceNames = popularServices.map(s => s.name);
  ...
  const filteredServices = availableServices.filter(service => {
    if (!serviceQuery || serviceQuery.length < 1) return true;
    const normalizedQuery = normalizeText(serviceQuery);
    const normalizedService = normalizeText(service);
    return normalizedService.includes(normalizedQuery);
  }).slice(0, 10);
      ...
      <div id="salon-results" className="flex-grow bg-gray-50">
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-100 p-4 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end mb-3 md:mb-0">
              ...
              {/* Service Filter - Autocomplete (NEW in V2) */}
              <div className="md:col-span-1">
                <label className="hidden md:block text-xs font-medium text-gray-600 mb-1.5">Usluga</label>
                <div className="relative" ref={serviceInputRef}>
                  <input
                    type="text"
                    value={filters.service || serviceQuery}
                    onChange={(e) => {
                    ...
                    <div className="absolute z-50 w-full mt-1 bg-white rounded-xl shadow-lg border border-gray-200 max-h-60 overflow-y-auto">
                      ...
                      {filteredServices.map((service, index) => (
                        <button
                          ...
                          className={`w-full px-4 py-2.5 text-left hover:bg-pink-50 transition-colors ${
                            filters.service === service ? 'bg-pink-100 text-pink-700' : 'text-gray-700'
                          }`}
                        >
                          {service}
                        </button>
              ...
              {/* Date Filter - European format date picker */}
